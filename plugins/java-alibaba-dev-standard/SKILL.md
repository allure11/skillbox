...
16. 类引用必须通过 `import` 导入后使用（无命名冲突时**禁止**内联全限定名，如 `java.util.Map map = new java.util.HashMap()`）；仅当存在同名类冲突时才允许使用全限定名区分。（补充强制约定）

### 日期时间
17. 日期 pattern 年份用小写 `yyyy`；区分 `M`（月）/`m`（分）、`H`（24h）/`h`（12h）。
18. 获取当前毫秒用 `System.currentTimeMillis()`，不用 `new Date().getTime()`。
19. 禁止使用 `java.sql.Date`/`java.sql.Time`；不写死一年 365 天。

### 集合处理
20. 集合判空用 `isEmpty()`，不用 `size()==0`。
21. 禁止在 foreach 中 `remove`/`add` 元素，用 Iterator 的 remove。
22. `Arrays.asList()` 返回的集合不能增删（不可变）；`ArrayList.subList` 结果不能强转 ArrayList、且父集合增删会导致子列表遍历异常。
23. `Collectors.toMap()` 注意 value 不能为 null（会 NPE）、key 重复会抛 IllegalStateException。
24. `Collections.emptyList()/singletonList()` 等返回 immutable 集合，不能增删。
25. `keySet()/values()/entrySet()` 返回的集合视图不能增删；`addAll()` 前先判空；集合转数组用 `toArray(T[])`。

### 并发处理
26. 线程池**禁止**用 `Executors` 创建，必须 `new ThreadPoolExecutor(...)` 显式指定参数；线程必须有意义命名；禁止直接 `new Thread`。
27. `SimpleDateFormat` 线程不安全，禁止 static 共享；用 `DateTimeFormatter` 或局部实例。
28. `ThreadLocal` 必须回收（`remove()`），尤其线程池复用场景；`ThreadLocal` 用 static 修饰。
29. 多资源/多表加锁保持一致的加锁顺序；锁内代码块小、锁外处理耗时逻辑；用锁的 try-finally 保证解锁，`lock.lock()` 在 try 外、`unlock()` 在 finally。
30. 高并发避免用「等于」判断做中断/退出条件；`volatile` 解决可见性但不保证原子性。

### 控制语句与注释
31. `if/else/for/while/do` 必须加大括号；switch 的每个 case 必须 break/return 终止（或注释说明）。
32. 三目运算符两分支类型对齐，避免 NPE（自动拆箱陷阱）。
33. 类/属性/方法用 Javadoc（`/** */`）；所有类必须有创建者与创建日期；抽象方法必须 Javadoc；枚举字段必须有注释。

### 异常与日志
34. 禁止 catch 后不处理（至少 `log.error("...", e)` 并给出原因）；不用异常做流程控制。
35. 资源/流对象在 finally 中关闭（或 try-with-resources）；**finally 中禁止 return**（吞掉异常）。
36. 事务场景 catch 到异常需回滚时必须手动回滚（`TransactionAspectSupport.currentTransactionStatus().setRollbackOnly()` 或显式 rollback）。
37. 调用 RPC/二方包/动态代理方法，catch 用 `Throwable`（或 Exception+Error 分类处理），勿只 catch RuntimeException。
38. 日志用 SLF4J 占位符 `log.info("name={}", name)`，禁止字符串拼接；禁止 `System.out` 打日志；trace/debug/info 前做级别开关判断；日志禁止直接 JSON 序列化整个对象；配置 `additivity=false` 防重复打印；异常日志必须含案发现场信息+堆栈。

### 安全规约
39. SQL 参数严格使用预编译绑定（`#{}`），禁止 `String` 拼接 SQL 或 `${}`；用户输入必须参数校验。
40. 用户敏感数据展示必须脱敏；个人页面/功能必须权限校验；输出到 HTML 前转义防 XSS；表单/AJAX 必须 CSRF 校验；URL 外部重定向必须白名单。
41. 短信/邮件/支付/下单等平台资源必须防重放（幂等/令牌）。

### MySQL 与 ORM
42. 表名、字段名小写，禁数字开头、禁双下划线中间只含数字、表名不用复数、禁用保留字；必备字段 `id`/`create_time`/`update_time`。
43. 布尔字段用 `is_xxx` + `unsigned tinyint`（POJO 侧去 is，resultMap 映射）；小数用 `decimal` 禁 float/double；varchar 超 5000 用 text 拆分。
44. 索引命名：主键 `pk_字段`、唯一 `uk_字段`、普通 `idx_字段`；唯一特性字段必须建唯一索引；varchar 索引必须指定长度；禁止左模糊/全模糊搜索（`like '%x'`）。
45. 超过三张表禁止 join；多表查询列名前必须加表别名（t1/t2...）；**禁止 select \***（明确字段列表）；用 `count(*)` 不用 `count(列名)`；禁止外键与级联（应用层解决）；禁止存储过程。
46. ORM 参数用 `#{}` 禁 `${}`；禁止 HashMap/Hashtable 直接作查询结果输出；更新记录必须同步更新 `update_time`；不用 `resultClass` 当返回参数。

### 设计规约
47. 存储方案与数据结构设计须评审通过并沉淀文档；业务状态 >3 个用状态图、调用链对象 >3 个用时序图、模型 >5 个用类图表达。

## 交付前自查清单（必须逐项确认后才能交付）

逐条自问，任一为「否」都不得交付，先修正：
1. 命名是否符合 UpperCamel/lowerCamel/全大写下划线，无拼音、无中文、无魔法值？类引用是否均 import 导入（无命名冲突时无全限定名内联）？
